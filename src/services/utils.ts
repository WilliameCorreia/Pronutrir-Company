...
export async function getUnidade(): Promise<IUnidade | null> {
...
export async function getPerfil(): Promise<IPerfis | null> {
  const user = await AsyncStorage.getItem('@Perfil');
  return user != null ? JSON.parse(user) : null;
}

export async function getImpressora(): Promise<IDevices | null> {
